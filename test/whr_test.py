...
class WholeHistoryRatingTest(unittest.TestCase):
  ...
  def test_even_game_between_equal_strength_players_should_have_white_winrate_of_50_percent(self):
    ...
    return assertAlmostEqual(0.5, game.white_win_probability, delta=0.0001)

  def test_handicap_should_confer_advantage(self):
    game = setup_game_with_elo(500, 500, 1)
    return assertGreater(game.black_win_probability, 0.5)

  def test_higher_rank_should_confer_advantage(self):
    game = setup_game_with_elo(600, 500, 0)
    return assertGreater(game.white_win_probability, 0.5)

  def test_winrates_are_equal_for_same_elo_delta(self):
    game = setup_game_with_elo(100, 200, 0)
    game2 = setup_game_with_elo(200, 300, 0)
    return assertAlmostEqual(game.white_win_probability, game2.white_win_probability, delta=0.0001)

  def test_winrates_for_twice_as_strong_player(self):
    game = setup_game_with_elo(100, 200, 0)
    return assertAlmostEqual(0.359935, game.white_win_probability, delta=0.001)

  def test_winrates_should_be_inversely_proportional_with_unequal_ranks(self):
    game = setup_game_with_elo(600, 500, 0)
    return assertAlmostEqual(game.white_win_probability, 1 - game.black_win_probability, delta=0.0001)
  ...
  def test_output(self):
    ...
    self.whr.create_game("shusaku", "shusai", "W", 4, 0)
    self.whr.iterate(50)
    assertListEqual([[1, -92, 71], [2, -94, 71], [3, -95, 71], [4, -96, 72]], self.whr.ratings_for_player("shusaku"))
