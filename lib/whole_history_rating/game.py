...
class Game:
  ...
  def prediction_score(self):
    if(self.white_win_probability() == 0.5):
      ...
      if((self.winner == "W" and self.white_win_probability() > 0.5) or (self.winner == "B" and self.white_win_probability() < 0.5)):
        return 1.0
      else:
        return 0.0

  def inspect(self):
    return("{}: W:{}(r={}) B:{}(r={}) winner = {}, komi = {}, handicap = {}".format(self, white_player.name, wpd.r if wpd else '?', black_player.name, bpd.r if bpd else '?', self.winner, self.komi, self.handicap))

  #def likelihood(self):
  #  return(white_win_probability if winner == "W" else 1 - self.white_win_probability)
